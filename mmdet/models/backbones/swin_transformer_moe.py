    ...
    def forward(self, x, H, W):
        ...
        for blk in self.blocks:
            ...
            if self.use_checkpoint:
                out = checkpoint.checkpoint(blk, x, attn_mask)
            else:
                out = blk(x, attn_mask)
        ...
        if self.downsample is not None:
            ...
            Wh, Ww = (H + 1) // 2, (W + 1) // 2
            return x, H, W, x_down, Wh, Ww, l_aux
        else:
            return x, H, W, x, H, W, l_aux
...
class SwinTransformerMoE(nn.Module):
    ...
    def __init__(self, pretrain_img_size=224, patch_size=4, in_chans=3,
                 ...
                 mlp_fc2_bias=True, init_std=0.02, pretrained_window_sizes=[0, 0, 0, 0],
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            layer = self.layers[i]
            x_out, H, W, x, Wh, Ww,cur_l_aux = layer(x, Wh, Ww)
            l_aux = cur_l_aux + l_aux
            ...
            if i in self.out_indices:
                ...
                outs.append(out)
        k=tuple(outs)
        return tuple(outs),l_aux * self.aux_loss_weight
    ...
    def train(self, mode=True):
        ...
        # TraceStack()
        super(SwinTransformerMoE, self).train(mode)
        self._freeze_stages()
import sys
def TraceStack():
	print ("--------------------")
